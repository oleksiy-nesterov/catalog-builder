...
import type { PageMeta, PageRepeat, PaperSize, Publication } from "../types";
...
type ValidatedPublication = Publication & {
...
export class PublicationApi {
  ...
  private static optionalString(source: Record<string, unknown>, key: string, label: string): string | undefined {
    const value = source[key];
  ...
  private static requirePaperSize(source: Record<string, unknown>, key: string, label: string): PaperSize {
    const value = PublicationApi.requireString(source, key, label);
  ...
  private static requireRecord(value: unknown, label: string): Record<string, unknown> {
    if (!PublicationApi.isRecord(value)) {
  ...
  private static expandPage(value: unknown, label: string): ExpandedPage[] {
    ...
    const record = PublicationApi.requireRecord(value, label);
    const id = PublicationApi.requireString(record, "name", label);
    const count = PublicationApi.requirePositiveInteger(record, "repeat", label);
  ...
  private static expandPages(values: unknown[], label: string): ExpandedPage[] {
    return values.flatMap((page, index) => PublicationApi.expandPage(page, `${label}: Page ${index}`));
  }

  private static validatePublication(value: unknown, label: string): ValidatedPublication {
    const record = PublicationApi.requireRecord(value, label);
    ...
    const expandedPages = PublicationApi.expandPages(pages, `${label}: "pages"`);

    return {
      title: PublicationApi.requireString(record, "title", label),
      language: PublicationApi.requireString(record, "language", label),
      edition: PublicationApi.optionalString(record, "edition", label),
      paperSize: PublicationApi.requirePaperSize(record, "paperSize", label),
  ...
  static async load(paths: PublicationPaths): Promise<LoadedPublication> {
    const publicationLabel = `publications/${paths.name}/publication.json`;
    const publication = PublicationApi.validatePublication(await PublicationApi.readJson(paths.publicationPath, publicationLabel), publicationLabel);

    const pages: LoadedPage[] = [];

    for (const [index, page] of publication.expandedPages.entries()) {
      const id = page.id;
