# Fixed-Page Catalog Authoring Guide

This guide is for fixed-page editorial and layout work: page composition, content structure, reusable templates and design-system usage. Technical setup, commands and build behavior live in [README.md](README.md). For flow-page book publications where Vivliostyle splits long content into physical pages, use [BOOK-GUID.md](BOOK-GUID.md).

The `catalogue` publication is a demo of a fixed-page catalogue. It shows one possible structure, but it is not a required model. A real project can be a product catalogue, magazine, brochure, lookbook, card set, report or any other paginated web/PDF publication.

## Build And Preview

Build the example catalogue publication:

```bash
npm run build:web -- catalogue
npm run build:pdf -- catalogue
npm run build:print -- catalogue
...
- `http://localhost:5173/catalogue/cover-front`

## Publication Root

Each publication lives in its own folder under `publications/`.
...
When changing the page format, keep the publication setting and print CSS in sync:
...
Use an empty string in `publication.json` for an intentional blank print page:
